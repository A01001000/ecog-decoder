...
## Hypothesis
The amplitude of response within goal-directed face perception is expected to be stronger. Additionally, we predict face perception to degrade less than house perception as noise levels increase. 

## Methods
We analyze human ECoG from 7 participants across two paradigms: a passive-viewing task and a goal-directed face-detection task, the latter requiring a button press to report target detection under varying visual noise levels (0–100% in 5% increments). For analysis, we apply cluster permutation analysis in order to find electrodes that significantly react to either faces or houses. We analyze spatial and temporal activity dynamics on those specific electrodes, both in the time and frequency domains. This is in addition to behavioural response evaluation. Next, we apply logistic regression and linear discriminant classifiers to decode whether participants were viewing faces or houses from ECoG broadband activity, specifically tuned in terms of goal-directedness and noise. Using these decoding models, we investigate the representation of the stimuli across different stages of visual perception. 
